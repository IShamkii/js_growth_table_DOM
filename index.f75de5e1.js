...
addEventListener("click", (e)=>{
    ...
    if (e.target === controls.appendRow && countRows.length < 10) table.lastElementChild.append(row.cloneNode(true));
    if (e.target === controls.removeRow && countRows.length > 2) row.remove();
    if (e.target === controls.appendColumn && countColumns.length < 10) countRows.forEach((el)=>el.append(document.createElement("td")));
    if (e.target === controls.removeColumn && countColumns.length > 2) countRows.forEach((el)=>el.querySelector("td").remove());
    updateControls();
});
updateControls();
